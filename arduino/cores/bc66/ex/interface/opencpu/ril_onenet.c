...
s32  RIL_QONENET_Create(void)
{
	...
	RIL_ONENET_DEBUG(DBG_Buffer,"<-- Send AT:%s,error=(%d),ret=(%d) -->\r\n",strAT,onenet_param.error,ret);
	if(RIL_AT_SUCCESS != ret)
    {
        return ret;
    }
    else if(0 != onenet_param.error) 
    {
        return onenet_param.error;// error number
    }
