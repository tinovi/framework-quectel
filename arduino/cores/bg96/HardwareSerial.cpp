...
void HardwareSerial::begin(qapi_UART_Open_Config_t *c)
{
	int r;
	ctor();
	if ((r = tx_event_flags_set(event, 0, TX_AND)))
	{
		DEBUG_UART("[SERIAL][%d] tx_event_flags_set = %d, %x\n", id, r, event);
		abort();
	}
	if (NULL == c)
	{
		DEBUG_UART("[SERIAL][%d] error\n", id);
		abort();
	}
	if (uart)
	{
		DEBUG_UART("[SERIAL][%d] alredy\n", id);
		return;
	}
	rx_ring_tail = 0;
	rx_ring_head = 0;
	c->rx_CB_ISR = (qapi_UART_Callback_Fn_t)&receiveCallback;
	c->tx_CB_ISR = (qapi_UART_Callback_Fn_t)&transmitCallback;
	if ((r = qapi_UART_Open(&uart, id, c)))
	{
